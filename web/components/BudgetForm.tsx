...
interface Props {
  onSubmit: (budget: number, startPoints: number, params: Params) => void;
  loading: boolean;
  initialParams?: Params;
}

export default function BudgetForm({ onSubmit, loading, initialParams }: Props) {
  ...
  function validate() {
    ...
    const pointBalance = parseInt(startPoints, 10);
    ...
    if (!Number.isInteger(budgetValue) || budgetValue < 1) nextErrors.budget = "1以上の整数で入力してください。";
    if (!Number.isInteger(pointBalance) || pointBalance < 0) nextErrors.startPoints = "0以上の整数で入力してください。";
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    onSubmit(parseInt(budget, 10), parseInt(startPoints, 10), {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <FieldNumber label="予算" value={budget} onChange={setBudget} error={errors.budget} suffix="円" min={1} />
        <FieldNumber label="開始時スペシャルクーポン" value={startPoints} onChange={setStartPoints} error={errors.startPoints} suffix="円" min={0} />
      </div>
